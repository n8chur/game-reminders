...
Version 0.0.1 is unsigned. Download it only from this repository's Releases page, inspect the warning, and use **More info > Run anyway** if the source and filename are correct.
...
Pull requests build and validate the app, Shortcut, portable ZIP, and Inno Setup installer. Pushing a tag that exactly matches the centralized version—for example `v0.0.1`—publishes both Windows packages to a GitHub Release.
