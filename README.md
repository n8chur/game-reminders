...
3. In first-run setup, select the `Shortcuts` folder inside iCloud Drive. The app finds it automatically when there is one unambiguous standard location, then creates or opens the required `Game Reminders` subfolder.
4. The app verifies and requests Windows' **Always keep on this device** preference for that subfolder. If iCloud or Windows rejects the request, setup explains how to apply it in File Explorer before retrying. Optionally enable **Launch Game Reminders when I sign in to Windows** during setup or toggle **Launch at login** later in the main window.
...
6. Download and import [`shortcuts/Game Reminder.shortcut`](shortcuts/Game%20Reminder.shortcut). It uses the built-in iCloud Shortcuts container with no folder prompts or device-specific bookmarks, so the same Shortcut works across Mac and iPhone. Create a reminder, then launch the configured executable.
...
Valid alias requests synchronized into `alias-requests/inbox` are added to the selected game
automatically and archived as accepted. Only requests that cannot be applied appear under
**Alias requests needing attention** in **My Games**, where they can be retried after correcting
the problem or rejected without changing the catalog. Conflicts preserve the original files for
recovery. A sample request is available in
