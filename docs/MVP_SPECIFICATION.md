...
Each reminder is an immutable JSON file with schema version 1, UUID, stable game ID, display name at creation, message, and creation timestamp. The Shortcut stages the serialized reminder with a non-JSON temporary extension in `inbox`, then renames it to `<UUID>.json` without overwrite. It reports success only after finalization succeeds and never modifies the pending file afterward.
