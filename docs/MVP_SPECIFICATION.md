...
An ambiguous or missing match is left unconfigured and marked **ACTION REQUIRED**. Its editor identifies the executable-path field that must be resolved and disables **Save** until at least one executable has been selected or entered. Detected paths support ordinary clicking, Ctrl/Shift multi-selection, and an explicit per-row add action. Candidate paths remain available after selection so the mapping can be corrected later. Manual games offer an executable file picker and store selected files as absolute paths. Existing filename-only mappings remain supported, and absolute paths may still be entered manually.
